...
public class DateTimeUtil {
	...
	public static java.util.Date getTFormatDate2(String date){
	 ...
	 */
	public static java.util.Date getFormatTime(String currDate) throws ParseException {
	 ...
	 * @return int 相差的分钟数
	 */
	public static int getMinutessBetweenDates(java.util.Date first, java.util.Date second) {
...
//		}
	...
	public static String getCansellTime(String busytimes, int days) {
		...
		java.util.Date lastDate = getFormatDateTime(getFormatDate(getDateBeforeOrAfter(29)), DATE_FORMAT);
		java.util.Date beginDate = getFormatDateTime(getFormatDate(getDateBeforeOrAfter(2)), DATE_FORMAT);
	 ...
	 * 相关说明：获取当前前后某天的日期
	 * 开发者：汤云涛
	 * 时间：2015年6月18日 上午10:35:49
	 */
	public static String addOrLessDay(int cursor, String format){
		
		SimpleDateFormat sf = new SimpleDateFormat(format);
		Calendar cal = Calendar.getInstance();
		cal.setTime(new Date());
        cal.add(Calendar.DAY_OF_YEAR,cursor);
        return sf.format(cal.getTime());  
	}
	
	public static String getBeforeOneDay(String format) {
		Calendar cal = Calendar.getInstance();
		cal.add(Calendar.DAY_OF_YEAR, -1);
		return getFormatDate(cal.getTime(), format);
	}
